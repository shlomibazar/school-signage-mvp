...
export default function ScreenLayoutEditor({
  ...
  // ── Layout changes (header / footer toggles, section count) ──
  ...
  // ── Section content updated by SectionEditor / ContentEditor ──
  const handleSectionUpdate = useCallback((updated: ScreenSection) => {
    setSections(prev => prev.map(s => s.id === updated.id ? updated : s))
    // Content saves go direct to the API — no need to mark layout dirty
  }, [])

  // ── Save layout (structure only, NOT content items) ──
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <div className="w-64 shrink-0 flex flex-col gap-3 overflow-y-auto">
          {/* Tab bar */}
          <div className="flex bg-gray-100 rounded-lg p-1 text-xs shrink-0">
          ...
          {activeTab === 'main' && (
            <div className="space-y-3">
              ...
              <div className="bg-white border border-gray-100 rounded-xl overflow-hidden shrink-0">
                    ...
                    {([1, 2, 3] as const).map(n => (
                      ...
                      >
                        {Array.from({ length: n }).map((_, i) => (
                          <span
                            key={i}
                            className={`block rounded-sm h-6 ${
                              layout.mainSections === n ? 'bg-blue-400' : 'bg-gray-300'
                            }`}
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <p className="text-[10px] text-gray-400 mt-2 text-center">
            לתצוגה מלאה לחץ על &quot;תצוגה ציבורית&quot; · שמירת תוכן אינה מחייבת שמירת פריסה
          </p>
        </div>
      </div>
    </div>
